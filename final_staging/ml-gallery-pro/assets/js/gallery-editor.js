(function () {
	...
	function defaultGalleryDisplaySettings() {
		...
		const defaults = {
			...
			cardGap: 18,
			roundedCorners: toggleValue(settings.rounded_corners, 1),
			...
			showHeading: toggleValue(settings.show_gallery_heading, 1),
			showDescription: toggleValue(settings.show_gallery_description, 1),
		...
		const presetId = String(settings.default_gallery_preset || "clean-grid");
	...
	function renderGalleryRow(item) {
		...
		const cover = item.cover && item.cover.thumb_url
			? `<img src="${escapeHtml(item.cover.thumb_url)}" alt="${escapeHtml(item.title || "")}">`
			: `<div class="mlgp-gallery-row__placeholder">${Number(item.item_count || 0) ? "IMG" : "SEM"}</div>`;
		const shortcode = galleryShortcode(item);

		return `
			<div class="mlgp-gallery-row ${isActive ? "is-active" : ""}">
				<div class="mlgp-gallery-row__media">${cover}</div>
				<div class="mlgp-gallery-row__main">
					<div class="mlgp-gallery-row__titlebar">
					...
					<div class="mlgp-list__meta">
						...
						<span>modo: ${escapeHtml(displayTypeLabel(item.display_type || "grid"))}</span>
						<span>${escapeHtml(item.updated_at || "Sem data")}</span>
					</div>
				</div>
				<div class="mlgp-gallery-row__shortcode">
					${renderCopyableShortcode(shortcode)}
				</div>
				<div class="mlgp-gallery-row__actions">
					<button type="button" class="mlgp-button mlgp-button--accent" data-mlgp-open-editor="${Number(item.id || 0)}">Gerenciar</button>
	...
	function renderItemCard(item, index) {
		...
		return `
			<article class="mlgp-media-card ${item.is_cover ? "is-cover" : ""} ${selected ? "is-selected" : ""}" data-item-id="${item.id}" data-mlgp-drop-item="${item.id}">
				<div class="mlgp-media-card__selection">
				...
				<div class="mlgp-media-card__aside">
					<div class="mlgp-actions mlgp-actions--vertical">
				...
				<div class="mlgp-add-images-layout">
					<section class="mlgp-editor-panel">
						...
						<div class="mlgp-add-images-page__footer">
							<div class="mlgp-modal__footer-meta">${isServerSource
								? `${escapeHtml(sourceLabel)}: ${escapeHtml(normalizeServerPath(state.createModal.serverPath) || "Informe a pasta relativa")}`
								: (hasCreateSelection() ? `${escapeHtml(sourceLabel)}: ${getCreateSelectionCount()} ${escapeHtml(config.strings.imagesSelected || "arquivo(s) selecionado(s)")}` : "Selecione as imagens primeiro e a galeria sera gerada automaticamente no envio.")}</div>
							<div class="mlgp-actions">
							<button type="button" class="mlgp-button mlgp-button--accent" data-mlgp-submit-create-gallery="1">${escapeHtml(getCreateSubmitLabel())}</button>
							</div>
						</div>
					...
					<aside class="mlgp-editor-panel mlgp-add-images-page__aside">
						...
						<div class="mlgp-add-images-steps">
							<div class="mlgp-add-images-step"><strong>1.</strong><span>Defina o titulo e o status da nova galeria.</span></div>
							<div class="mlgp-add-images-step"><strong>2.</strong><span>Envie arquivos do computador, uma pasta local inteira, um ZIP ou importe uma pasta do servidor.</span></div>
							<div class="mlgp-add-images-step"><strong>3.</strong><span>O plugin cria a galeria, salva no storage proprio e entrega o shortcode pronto.</span></div>
						</div>
						<div class="mlgp-shortcode-box">
							<span>Storage</span>
							<code class="mlgp-shortcode-box__code mlgp-shortcode-box__code--static">${escapeHtml(config.storageLabel || "wp-content/ml-gallery")}</code>
						</div>
						${activeShortcode ? `
		...
		if (isAddImagesPage) {
			...
			root.innerHTML = `
				...
				<section class="mlgp-panel mlgp-gallery-dashboard">
					...
					<div class="mlgp-gallery-table">
						<div class="mlgp-gallery-table__head">
							<span>Miniatura</span>
							<span>Galeria</span>
							<span>Shortcode</span>
							<span>Acoes</span>
						</div>
						<div class="mlgp-gallery-table__body">
	...
	async function refreshGalleries(shouldRender = true) {
		const response = await request("mlgp_list_galleries", {});
	...
	root.addEventListener("click", async (event) => {
		...
		// Resolution of specific buttons/cards for the create modal flow
		const closeBtn = event.target.closest("[data-mlgp-close-create-modal]");
		const submitBtn = event.target.closest("[data-mlgp-submit-create-gallery]");
		const sourceBtn = event.target.closest("[data-mlgp-open-create-source]");
		...
		if (sourceBtn) {
			state.createModal.source = sourceBtn.dataset.mlgpOpenCreateSource;
		...
		const target = event.target.closest("[data-mlgp-open-create-modal],[data-mlgp-open-editor],[data-mlgp-close-editor],[data-mlgp-delete-gallery],[data-mlgp-copy-shortcode],[data-mlgp-open-upload],[data-mlgp-import-server],[data-mlgp-save-all],[data-mlgp-set-cover],[data-mlgp-remove-item],[data-mlgp-move-item],[data-mlgp-select-all],[data-mlgp-clear-selection],[data-mlgp-clear-item-filters],[data-mlgp-bulk-action],[data-mlgp-apply-preset],[data-mlgp-bulk-tab]");
