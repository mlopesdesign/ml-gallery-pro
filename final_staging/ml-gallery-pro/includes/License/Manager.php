...
final class Manager {
	...
	public function validate_license( string $license_key ): array {
		...
		$action         = $normalized_key === '' ? 'start_trial' : 'validate_license';
	...
	public function build_payload(): array {
		...
		return [
			...
			'plan'           => (string) ( $state['plan'] ?? ( $this->is_full_license_active() ? 'Full' : 'Free' ) ),
	...
	private function build_state_from_response( array $response, string $license_key, string $action ): array {
		...
		$state['status']          = isset( $response['status'] ) ? (string) $response['status'] : 'unknown';
		$state['message']         = isset( $response['message'] ) ? (string) $response['message'] : '';
		$state['license_source']  = isset( $response['license_source'] ) ? (string) $response['license_source'] : ( 'start_trial' === $action ? 'server_trial' : ( '' === $license_key ? 'free' : 'license' ) );
		$state['plan']            = isset( $response['plan'] ) ? (string) $response['plan'] : ( 'free' === $state['license_source'] ? 'Free' : 'Full' );

		if ( isset( $response['expires_at'] ) ) {
			$state['expires_at'] = (string) $response['expires_at'];
		}
