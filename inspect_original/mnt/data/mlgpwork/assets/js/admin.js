(function () {
	...
	const page = root.dataset.page || config.page || "dashboard";
	const DEFAULT_HEADING_FONT_SIZE = 34;

	if (page === "galleries" || page === "add-images") {
		return;
	}
	...
	function defaultAlbumDisplaySettings(input = {}, globalSettings = config.settings || {}) {
		...
		return {
			displayType: String(source.display_type || source.displayType || "grid"),
			columnsDesktop: Number(source.columns_desktop ?? source.columnsDesktop ?? globals.columns_desktop ?? 4),
			columnsTablet: Number(source.columns_tablet ?? source.columnsTablet ?? globals.columns_tablet ?? 3),
			columnsMobile: Number(source.columns_mobile ?? source.columnsMobile ?? globals.columns_mobile ?? 2),
			cardGap: Number(source.card_gap ?? source.cardGap ?? globals.card_gap ?? 18),
			...
			roundedCorners: Number(source.rounded_corners ?? source.roundedCorners ?? 0),
			paginationEnabled: Number(source.pagination_enabled ?? source.paginationEnabled ?? globals.pagination_enabled ?? 1),
			itemsPerPage: Number(source.items_per_page ?? source.itemsPerPage ?? globals.items_per_page ?? 24),
			showHeading: Number(source.show_heading ?? source.showHeading ?? 0),
			showDescription: Number(source.show_description ?? source.showDescription ?? 0),
			showTitles: Number(source.show_titles ?? source.showTitles ?? 1),
			showCaptions: Number(source.show_captions ?? source.showCaptions ?? globals.show_captions ?? 0),
	...
	function getAvailableAlbumGalleries() {
		...
		return (state.albumEditor.availableGalleries || []).filter((item) => !selectedKeys.has(`gallery:${Number(item.id || 0)}`));
	}
	...
	function getAvailableSubalbums() {
		...
		return (state.albumEditor.availableAlbums || []).filter((item) => !selectedKeys.has(`album:${Number(item.id || 0)}`));
	}
	...
	function renderDashboardLegacy() {
		...
		<h2>Estado da base v${escapeHtml(config.version || "0.15.0")}</h2>
		...
		root.innerHTML = `
			<section class="mlgp-card-panel mlgp-dashboard-hero">
				<div class="mlgp-dashboard-hero__lead">
					...
					<h2>Galerias, albuns e shortcodes sob o mesmo fluxo de entrega.</h2>
					<p>O admin agora entra mais como produto: organizacao visual, storage proprio, update validado e um ponto claro para seguir lapidando a experiencia comercial.</p>
					<div class="mlgp-dashboard-hero__stats">
						<span class="mlgp-dashboard-hero__stat">${escapeHtml(formatCount(totalCollections))} estruturas criadas</span>
						<span class="mlgp-dashboard-hero__stat">${escapeHtml(formatCount(totalAssets))} registros operacionais</span>
						<span class="mlgp-dashboard-hero__stat">v${escapeHtml(config.version || "0.22.9")}</span>
					</div>
				</div>
				<div class="mlgp-dashboard-hero__aside">
					...
					<div class="mlgp-dashboard-signal is-soft">
						<span class="mlgp-dashboard-signal__label">Proximo passo recomendado</span>
			...
			<section class="mlgp-dashboard-grid mlgp-dashboard-grid--support">
				<article class="mlgp-card-panel mlgp-story-card">
					...
					<p class="mlgp-card-panel__intro">Uma leitura curta para manter o projeto andando sem perder a base que ja esta confiavel.</p>
					<div class="mlgp-story-list">
						...
						<div class="mlgp-story-step">
							...
							<div>
								...
								<p>Use a aba de validacao para checar update, storage, banco e ambiente antes de grandes ajustes visuais.</p>
							</div>
						</div>
						<div class="mlgp-story-step">
							...
							<div>
								...
								<p>Com a base protegida, o proximo ganho vem de polish em manager, colecoes e apresentacao do produto.</p>
							</div>
						</div>
					</div>
				</article>
				<article class="mlgp-card-panel mlgp-story-card">
					...
					<p class="mlgp-card-panel__intro">O painel ja opera com pilares importantes da linha profissional e agora fica mais facil enxergar o que vem a seguir.</p>
					...
					<div class="mlgp-story-callout">
						...
						<p>O plugin ja saiu da fase de sobrevivencia. O foco agora pode migrar para acabamento, confianca operacional e experiencia comercial do admin.</p>
					</div>
	...
	function renderAlbumRow(item) {
		...
		const cover = item.cover && item.cover.thumb_url
			? `<div class="mlgp-entity-row__thumb"><img src="${escapeHtml(item.cover.thumb_url)}" alt="${escapeHtml(item.title || "")}"></div>`
			: `<div class="mlgp-entity-row__thumb is-empty">ALB</div>`;

		return `
			<div class="mlgp-list__item mlgp-entity-row mlgp-list__item--selectable ${isActive ? "is-active" : ""}">
				<label class="mlgp-check mlgp-check--row">
				...
				<div class="mlgp-entity-row__main">
					...
					${item.shortcode ? renderCopyableShortcode(item.shortcode) : ""}
				</div>
				<div class="mlgp-actions">
	...
	function renderAlbumChooserItem(item, type) {
		const cover = item.cover && item.cover.thumb_url
			? `<div class="mlgp-linked-item__media"><img src="${escapeHtml(item.cover.thumb_url)}" alt="${escapeHtml(item.title || "")}"></div>`
		...
		return `
			<div class="mlgp-linked-item">
				...
				<div class="mlgp-actions">
					<button type="button" class="mlgp-button mlgp-button--accent" data-mlgp-add-album-item="${type}" data-item-id="${Number(item.id || 0)}">Adicionar</button>
				</div>
			</div>
		`;
	}
	...
	function renderAlbumManager() {
		...
		const availableGalleries = getAvailableAlbumGalleries();
		const availableAlbums = getAvailableSubalbums();
					...
					<div class="mlgp-editor-layout">
						...
						<div class="mlgp-editor-main">
							<section class="mlgp-editor-panel">
								<div class="mlgp-panel-header-inline">
									<div>
										<h3>Adicionar galerias</h3>
										<p class="mlgp-panel__intro">Vincule galerias ja criadas ao album atual.</p>
									</div>
								</div>
								<div class="mlgp-linked-list">
									${availableGalleries.length ? availableGalleries.map((item) => renderAlbumChooserItem(item, "gallery")).join("") : '<div class="mlgp-empty">Nenhuma galeria disponivel para adicionar.</div>'}
								</div>
							</section>
							<section class="mlgp-editor-panel">
								<div class="mlgp-panel-header-inline">
									<div>
										<h3>Adicionar subalbuns</h3>
										<p class="mlgp-panel__intro">Crie navegacao em cascata com outros albuns, sem permitir ciclos.</p>
									</div>
								</div>
								<div class="mlgp-linked-list">
									${availableAlbums.length ? availableAlbums.map((item) => renderAlbumChooserItem(item, "album")).join("") : '<div class="mlgp-empty">Nenhum subalbum disponivel para adicionar.</div>'}
								</div>
							</section>
							<section class="mlgp-editor-panel mlgp-editor-panel--wide">
								<div class="mlgp-panel-header-inline">
									<div>
										...
										<p class="mlgp-panel__intro">Defina a ordem final das galerias e subalbuns. A primeira capa valida da lista sera usada como destaque do album.</p>
									</div>
									<div class="mlgp-list__meta">
								...
								<div class="mlgp-linked-list">
									${items.length ? items.map((item, index) => renderAlbumLinkedItem(item, index)).join("") : '<div class="mlgp-empty">Nenhum item vinculado ainda. Use os blocos acima para montar o album.</div>'}
								</div>
							</section>
						</div>
					</div>
			...
			<section class="mlgp-panel mlgp-panel--wide">
				<div class="mlgp-panel-header-inline">
					...
					<div class="mlgp-actions mlgp-actions--bulk">
						...
						<div class="mlgp-list__meta">
							<span>${state.albums.length} albuns</span>
							<span>${totalLinkedItems} itens vinculados</span>
							${activeAlbum ? `<span>editando ID ${Number(state.albumEditor.activeId || 0)}</span>` : ""}
						</div>
						${activeAlbum ? '<button type="button" class="mlgp-button mlgp-button--ghost" data-reset-album="1">Novo album</button>' : ""}
					</div>
				</div>
				<div class="mlgp-list">
					${state.albums.length ? state.albums.map(renderAlbumRow).join("") : '<div class="mlgp-empty">Nenhum album cadastrado ainda.</div>'}
				</div>
			</section>
		`;

		root.innerHTML = `
			${activeAlbum ? albumsPanel : `<div class="mlgp-grid-two">${createPanel}${albumsPanel}</div>`}
			${renderAlbumManager()}
		`;
	...
	function renderSettingsGeneralTab(settings, storageLabel) {
		return renderSettingsSection(
			...
			`
				<div class="mlgp-settings-grid">
					...
					<div class="mlgp-field">
						...
						<select id="mlgp-default-gallery-preset" name="default_gallery_preset">
							<option value="clean-grid" ${String(settings.default_gallery_preset || "clean-grid") === "clean-grid" ? "selected" : ""}>Clean Grid</option>
							<option value="editorial-tile" ${String(settings.default_gallery_preset || "clean-grid") === "editorial-tile" ? "selected" : ""}>Editorial Tile</option>
							<option value="impact-mosaic" ${String(settings.default_gallery_preset || "clean-grid") === "impact-mosaic" ? "selected" : ""}>Impact Mosaic</option>
							<option value="story-justified" ${String(settings.default_gallery_preset || "clean-grid") === "story-justified" ? "selected" : ""}>Story Justified</option>
							<option value="showcase-filmstrip" ${String(settings.default_gallery_preset || "clean-grid") === "showcase-filmstrip" ? "selected" : ""}>Showcase Filmstrip</option>
						</select>
					</div>
				</div>
				<div class="mlgp-check">
				...
				<div class="mlgp-check">
					<input id="mlgp-rounded-corners" type="checkbox" name="rounded_corners" ${Number(settings.rounded_corners ?? 1) ? "checked" : ""}>
	...
	function renderSettingsContentTab(settings) {
		...
		return renderSettingsSection(
			...
			`
				...
				<div class="mlgp-check">
					<input id="mlgp-show-gallery-heading" type="checkbox" name="show_gallery_heading" ${Number(settings.show_gallery_heading ?? 1) ? "checked" : ""}>
					<label for="mlgp-show-gallery-heading">Exibir título principal da galeria por padrão</label>
				</div>
				<div class="mlgp-check">
					<input id="mlgp-show-gallery-description" type="checkbox" name="show_gallery_description" ${Number(settings.show_gallery_description ?? 1) ? "checked" : ""}>
	...
	function renderSettingsAlbumsTab(settings) {
		return renderSettingsSection(
			"Capas dos álbuns",
			"Defina a janela visual padrão dos cards de álbum no frontend.",
			`
				<div class="mlgp-settings-grid">
					<div class="mlgp-field">
						<label for="mlgp-album-cover-width">Largura padrão da capa do álbum (px)</label>
						<input id="mlgp-album-cover-width" type="number" min="120" max="1800" name="album_cover_width" value="${Number(settings.album_cover_width || 360)}">
					</div>
					<div class="mlgp-field">
						<label for="mlgp-album-cover-height">Altura padrão da capa do álbum (px)</label>
						<input id="mlgp-album-cover-height" type="number" min="120" max="1200" name="album_cover_height" value="${Number(settings.album_cover_height || 280)}">
					</div>
					<div class="mlgp-field">
			...
			`
				<div class="mlgp-settings-grid">
					<div class="mlgp-field">
						...
						<input id="mlgp-thumb-width" type="number" min="80" max="2400" name="thumb_width" value="${Number(settings.thumb_width || 360)}">
					</div>
					<div class="mlgp-field">
						<label for="mlgp-thumb-height">Altura da miniatura (px)</label>
						<input id="mlgp-thumb-height" type="number" min="80" max="2400" name="thumb_height" value="${Number(settings.thumb_height || 360)}">
					</div>
					<div class="mlgp-field">
	...
	function renderLicensePanel(license) {
		...
		const plan = payload.plan || (payload.is_full_active ? "Full" : "Free");
		...
		return `
			<div class="mlgp-license-panel">
				<div class="mlgp-license-panel__header">
					...
					<span class="mlgp-pill">${escapeHtml(plan)}</span>
				</div>
				...
				<div class="mlgp-actions">
					<button type="button" class="mlgp-button mlgp-button--accent" data-mlgp-validate-license>${escapeHtml(config.strings.licenseValidate || "Validar serial")}</button>
		...
		root.innerHTML = `
			<section class="mlgp-panel">
				...
				<form id="mlgp-settings-form" class="mlgp-form">
					...
					<h3>Capa de exibicao dos albuns</h3>
					<p class="mlgp-panel__intro">Defina a janela visual padrao da capa dos cards de album. Se quiser manter a imagem inteira, use o encaixe sem recorte.</p>
					<div class="mlgp-settings-grid">
						<div class="mlgp-field">
							<label for="mlgp-album-cover-width">Largura padrao da capa do album (px)</label>
							<input id="mlgp-album-cover-width" type="number" min="120" max="1800" name="album_cover_width" value="${Number(settings.album_cover_width || 360)}">
						</div>
						<div class="mlgp-field">
							<label for="mlgp-album-cover-height">Altura padrao da capa do album (px)</label>
							<input id="mlgp-album-cover-height" type="number" min="120" max="1200" name="album_cover_height" value="${Number(settings.album_cover_height || 280)}">
						</div>
						<div class="mlgp-field">
							<label for="mlgp-album-cover-fit">Encaixe padrao da capa do album</label>
							<select id="mlgp-album-cover-fit" name="album_cover_fit">
								<option value="contain" ${String(settings.album_cover_fit || "contain") === "contain" ? "selected" : ""}>Mostrar imagem inteira</option>
								<option value="cover" ${String(settings.album_cover_fit || "contain") === "cover" ? "selected" : ""}>Preencher e recortar</option>
							</select>
					...
					<h3>Perfis de imagem</h3>
					<p class="mlgp-panel__intro">Defina os tamanhos gerados no storage proprio do plugin. Esses perfis alimentam thumb, manager, grid, slideshow e regeneracao.</p>
					<div class="mlgp-settings-grid">
						<div class="mlgp-field">
							<label for="mlgp-thumb-width">Thumb largura</label>
							<input id="mlgp-thumb-width" type="number" min="80" max="2400" name="thumb_width" value="${Number(settings.thumb_width || 360)}">
						</div>
						<div class="mlgp-field">
							<label for="mlgp-thumb-height">Thumb altura</label>
							<input id="mlgp-thumb-height" type="number" min="80" max="2400" name="thumb_height" value="${Number(settings.thumb_height || 360)}">
						</div>
						<div class="mlgp-field">
							<label for="mlgp-medium-width">Medium largura</label>
							<input id="mlgp-medium-width" type="number" min="120" max="3600" name="medium_width" value="${Number(settings.medium_width || 900)}">
						</div>
						<div class="mlgp-field">
							<label for="mlgp-medium-height">Medium altura</label>
							<input id="mlgp-medium-height" type="number" min="120" max="3600" name="medium_height" value="${Number(settings.medium_height || 900)}">
						</div>
						<div class="mlgp-field">
							<label for="mlgp-large-width">Large largura</label>
							<input id="mlgp-large-width" type="number" min="240" max="5200" name="large_width" value="${Number(settings.large_width || 1600)}">
						</div>
						<div class="mlgp-field">
							<label for="mlgp-large-height">Large altura</label>
							<input id="mlgp-large-height" type="number" min="240" max="5200" name="large_height" value="${Number(settings.large_height || 1600)}">
						</div>
					</div>
					...
					<div class="mlgp-check">
						<input id="mlgp-thumb-crop" type="checkbox" name="thumb_crop" ${Number(settings.thumb_crop || 0) ? "checked" : ""}>
						<label for="mlgp-thumb-crop">Aplicar crop fixo nas thumbnails</label>
					</div>

					<h3>Watermark</h3>
					<p class="mlgp-panel__intro">O watermark textual e aplicado automaticamente nas variantes de exibicao regeneradas a partir das imagens locais do plugin.</p>
					<div class="mlgp-check">
						<input id="mlgp-watermark-enabled" type="checkbox" name="watermark_enabled" ${Number(settings.watermark_enabled || 0) ? "checked" : ""}>
						<label for="mlgp-watermark-enabled">Ativar watermark nas imagens geradas</label>
					</div>
					<div class="mlgp-settings-grid">
						<div class="mlgp-field">
							<label for="mlgp-watermark-text">Texto do watermark</label>
							<input id="mlgp-watermark-text" type="text" name="watermark_text" value="${escapeHtml(settings.watermark_text || "")}" placeholder="ML Gallery Pro">
						</div>
						<div class="mlgp-field">
							<label for="mlgp-watermark-opacity">Opacidade (%)</label>
						...
						<div class="mlgp-field">
							<label for="mlgp-watermark-position">Posicao</label>
							<select id="mlgp-watermark-position" name="watermark_position">
								<option value="top-left" ${String(settings.watermark_position || "bottom-right") === "top-left" ? "selected" : ""}>Topo esquerdo</option>
								<option value="top-right" ${String(settings.watermark_position || "bottom-right") === "top-right" ? "selected" : ""}>Topo direito</option>
								<option value="bottom-left" ${String(settings.watermark_position || "bottom-right") === "bottom-left" ? "selected" : ""}>Base esquerda</option>
					...
					<h3>Labels e mensagens</h3>
					<div class="mlgp-field">
						<label for="mlgp-label-view-gallery">Label de galeria</label>
						<input id="mlgp-label-view-gallery" type="text" name="label_view_gallery" value="${escapeHtml(settings.label_view_gallery || "")}">
					</div>

					<div class="mlgp-field">
						<label for="mlgp-label-back-to-album">Label de retorno do album</label>
						<input id="mlgp-label-back-to-album" type="text" name="label_back_to_album" value="${escapeHtml(settings.label_back_to_album || "")}">
					</div>
					...
					<div class="mlgp-field">
						<label for="mlgp-empty-album-message">Mensagem de album vazio</label>
						<input id="mlgp-empty-album-message" type="text" name="empty_album_message" value="${escapeHtml(settings.empty_album_message || "")}">
					</div>

					<div class="mlgp-panel-header-inline">
						<div>
							<h3>Ferramentas operacionais</h3>
							<p class="mlgp-panel__intro">Use a rotina completa em lotes para apagar miniaturas/previews correspondentes, recriar do zero e revincular a biblioteca local em <code class="mlgp-inline-code">${escapeHtml(config.storageLabel || "wp-content/ml-gallery")}</code>.</p>
							${progressText ? `<p class="mlgp-panel__intro"><strong>${escapeHtml(progressText)}</strong>${progressPercent > 0 ? ` (${Math.min(100, progressPercent)}%)` : ""}</p>` : ""}
						</div>
						<div class="mlgp-actions">
							<button type="button" class="mlgp-button" data-mlgp-regenerate-all="1" ${isRegenerating ? "disabled" : ""}>${escapeHtml(isRegenerating ? "Reconstruindo miniaturas, previews e revinculando em lotes..." : "Reconstruir miniaturas, previews e revincular em lotes")}</button>
							<button type="button" class="mlgp-button mlgp-button--danger" data-mlgp-factory-reset="1">${escapeHtml(config.strings.factoryResetAction || "Resetar plugin para o estado de fabrica")}</button>
						</div>
					</div>

					<div class="mlgp-actions">
						<button type="submit" class="mlgp-button mlgp-button--accent">Salvar configuracoes</button>
					</div>
				</form>
			</section>
		`;
	...
	function addAlbumItem(itemType, itemId) {
	...
	async function loadAlbums(shouldRender = true) {
		const response = await request("mlgp_list_albums", {});
	...
	function handleRootFieldChange(event) {
	...
	root.addEventListener("click", async (event) => {
		...
		const target = event.target.closest("[data-edit-gallery],[data-delete-gallery],[data-reset-gallery],[data-manage-album],[data-delete-album],[data-reset-album],[data-mlgp-add-album-item],[data-mlgp-move-album-item],[data-mlgp-remove-album-item],[data-mlgp-save-album-all],[data-mlgp-copy-shortcode],[data-mlgp-regenerate-all],[data-mlgp-media-pick],[data-mlgp-media-clear],[data-mlgp-settings-tab],[data-mlgp-refresh-validation],[data-mlgp-validate-license],[data-mlgp-deactivate-license],[data-mlgp-delete-selected-galleries],[data-mlgp-delete-all-galleries],[data-mlgp-delete-all-images],[data-mlgp-delete-selected-albums],[data-mlgp-factory-reset]");
		...
		if (target.hasAttribute("data-mlgp-validate-license") || target.hasAttribute("data-mlgp-deactivate-license")) {
			...
			try {
				...
				state.license = response.license || state.license;
				showNotice(config.strings.licenseSuccess || "Licenca atualizada.");
				renderSettings();
			} catch (error) {
		...
		if (target.dataset.mlgpAddAlbumItem) {
			addAlbumItem(target.dataset.mlgpAddAlbumItem, Number(target.dataset.itemId || 0));
			return;
		}
