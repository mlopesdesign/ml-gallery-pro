...
final class Repository {
	...
	public function bulk_update_gallery_items( int $gallery_id, array $item_ids, string $action, array $options = [] ) {
		...
		if ( ! in_array( $action, [ 'show', 'hide', 'delete', 'append_tags', 'replace_tags', 'clear_tags', 'replace_titles', 'clear_titles', 'replace_captions', 'clear_captions', 'regenerate', 'rotate_left', 'rotate_right' ], true ) ) {
		...
		} elseif ( in_array( $action, [ 'replace_titles', 'clear_titles', 'replace_captions', 'clear_captions' ], true ) ) {
	...
	public function factory_reset() {
		...
		update_option( 'mlgp_license_state', [
			'license_key'    => '',
			'license_source' => 'free',
			'status'         => 'free',
			'message'        => '',
	...
	public function save_settings( array $input ): array {
		...
		$settings = [
			...
			'default_gallery_preset'=> $this->sanitize_choice(
				...
				[ 'clean-grid', 'editorial-tile', 'impact-mosaic', 'story-justified', 'showcase-filmstrip' ],
				'clean-grid'
			),
	...
	private function get_tag_rows( bool $published_only ): array {
		...
		if ( $published_only ) {
			$sql .= $wpdb->prepare( ' AND g.status = %s', 'publish' );
		}
