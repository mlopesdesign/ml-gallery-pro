...
final class Shortcodes {
	...
	private function render_gallery( int $gallery_id, array $atts ): string {
		...
		ob_start();
		?>
		<div class="mlgp-frontend mlgp-gallery mlgp-gallery--<?php echo esc_attr( $config['display_type'] ); ?>" style="<?php echo esc_attr( $wrapper_style ); ?>" data-mlgp-gallery-id="<?php echo esc_attr( (string) $gallery_id ); ?>">
	...
	private function render_album_collection_item( int $root_album_id, array $item, array $config, array $settings ): string {
		...
		<article class="mlgp-card mlgp-card--album mlgp-card--album-entry mlgp-card--<?php echo esc_attr( $config['display_type'] ); ?>" style="<?php echo esc_attr( $item_style ); ?>">
			<a class="mlgp-card__media mlgp-card__media--album<?php echo empty( $cover ) ? ' is-empty' : ''; ?>" href="<?php echo esc_url( $item_url ); ?>">
				<?php if ( $cover ) : ?>
			...
			<?php if ( ( empty( $config['hide_all_titles'] ) && ! empty( $config['show_titles'] ) ) || '' !== $caption ) : ?>
				<div class="mlgp-card__content mlgp-card__content--album">
					<?php if ( empty( $config['hide_all_titles'] ) && ! empty( $config['show_titles'] ) ) : ?>
						<h3><a class="mlgp-card__title-link" href="<?php echo esc_url( $item_url ); ?>"><?php echo esc_html( (string) $child['title'] ); ?></a></h3>
					<?php endif; ?>
					<?php if ( '' !== $caption ) : ?>
	...
	private function render_album_hero( array $album, array $items, array $settings ): string {
		...
		<section class="mlgp-album-hero">
			<div class="mlgp-album-hero__copy">
				<span class="mlgp-album-hero__eyebrow"><?php esc_html_e( 'Colecao', 'ml-gallery-pro' ); ?></span>
				...
				<div class="mlgp-album-hero__stats">
					...
					<?php if ( $album_count > 0 ) : ?>
						<span class="mlgp-album-hero__stat"><?php echo esc_html( sprintf( _n( '%d subalbum', '%d subalbuns', $album_count, 'ml-gallery-pro' ), $album_count ) ); ?></span>
					<?php endif; ?>
					<span class="mlgp-album-hero__stat"><?php echo esc_html( sprintf( _n( '%d item na colecao', '%d itens na colecao', $total_items, 'ml-gallery-pro' ), $total_items ) ); ?></span>
				</div>
			</div>
			<div class="mlgp-album-hero__visual">
				...
				<?php else : ?>
					<div class="mlgp-album-hero__empty"><?php esc_html_e( 'Colecao pronta para receber galerias e subalbuns.', 'ml-gallery-pro' ); ?></div>
				<?php endif; ?>
			</div>
		</section>
		<?php
	...
	private function build_gallery_config( array $gallery, array $global_settings, array $atts ): array {
		...
		return [
			...
			'gap'                => $this->sanitize_integer( $atts['gap'] ?: ( $gallery_settings['card_gap'] ?? 18 ), 0, 48 ),
	...
	private function build_wrapper_style( array $config ): string {
		$card_radius        = ! empty( $config['rounded_corners'] ) ? 16 : 0;
		$thumb_radius       = ! empty( $config['rounded_corners'] ) ? 12 : 0;
		...
		return sprintf(
			'--mlgp-cols-desktop:%1$d;--mlgp-cols-tablet:%2$d;--mlgp-cols-mobile:%3$d;--mlgp-gap:%4$dpx;--mlgp-card-radius:%5$dpx;--mlgp-thumb-radius:%6$dpx;--mlgp-justified-row-height:%7$dpx;--mlgp-heading-size:%8$dpx;--mlgp-heading-color:%9$s;--mlgp-item-title-size:%10$dpx;--mlgp-item-title-color:%11$s;--mlgp-album-cover-width:%12$dpx;--mlgp-album-cover-height:%13$dpx;--mlgp-album-cover-fit:%14$s;',
	...
	private function build_album_config( array $album, array $global_settings, array $atts ): array {
		...
		$display_type   = (string) ( $atts['display_type'] ?: ( $album['display_type'] ?? 'grid' ) );
		$legacy_mode    = sanitize_key( $display_type );

		return [
			'display_type'         => $this->sanitize_album_display_type( $display_type ),
			'columns_desktop'      => $this->sanitize_integer( $atts['columns_desktop'] ?: ( $album_settings['columns_desktop'] ?? $global_settings['columns_desktop'] ?? 4 ), 1, 8 ),
			'columns_tablet'       => $this->sanitize_integer( $atts['columns_tablet'] ?: ( $album_settings['columns_tablet'] ?? $global_settings['columns_tablet'] ?? 3 ), 1, 6 ),
			'columns_mobile'       => $this->sanitize_integer( $atts['columns_mobile'] ?: ( $album_settings['columns_mobile'] ?? $global_settings['columns_mobile'] ?? 2 ), 1, 4 ),
			'gap'                  => $this->sanitize_integer( $atts['gap'] ?: ( $album_settings['card_gap'] ?? $global_settings['card_gap'] ?? 18 ), 0, 48 ),
			'rounded_corners'      => $this->bool_value( $atts['rounded_corners'], $album_settings['rounded_corners'] ?? 0 ),
			'pagination_enabled'   => $this->bool_value( $atts['pagination'], $album_settings['pagination_enabled'] ?? $global_settings['pagination_enabled'] ?? 1 ),
			'items_per_page'       => $this->sanitize_integer( $atts['per_page'] ?: ( $album_settings['items_per_page'] ?? $global_settings['items_per_page'] ?? 18 ), 1, 5000 ),
			'show_heading'         => $this->bool_value( $atts['show_heading'], $album_settings['show_heading'] ?? 0 ),
			'show_description'     => $this->bool_value( $atts['show_description'], $album_settings['show_description'] ?? 0 ),
			'show_titles'          => $this->bool_value( $atts['show_titles'], $album_settings['show_titles'] ?? $global_settings['show_titles'] ?? 1 ),
			'show_captions'        => $this->bool_value( $atts['show_captions'], $album_settings['show_captions'] ?? $global_settings['show_captions'] ?? ( 'extended' === $legacy_mode ? 1 : 0 ) ),
			'hide_all_titles'      => ! empty( $global_settings['hide_all_titles'] ) ? 1 : 0,
			...
			'item_title_font_size' => $this->sanitize_integer( $atts['item_title_font_size'] ?: ( $album_settings['item_title_font_size'] ?? $global_settings['item_title_font_size'] ?? 18 ), 10, 48 ),
			'item_title_color'     => $this->sanitize_color( $atts['item_title_color'] ?: ( $album_settings['item_title_color'] ?? $global_settings['item_title_color'] ?? '#172033' ), '#172033' ),
		];
	}
	...
	private function render_album_gallery_view( int $root_album_id, int $gallery_id, array $trail, array $atts, array $album_config ): string {
		...
		<div class="mlgp-frontend mlgp-collection-view" style="<?php echo esc_attr( $wrapper_style ); ?>">
			...
			<div class="mlgp-collection-view__toolbar">
				<a class="mlgp-collection-back" href="<?php echo esc_url( $back_url ); ?>"><?php esc_html_e( 'Voltar para a colecao', 'ml-gallery-pro' ); ?></a>
			</div>
	...
	private function render_album_breadcrumb( int $root_album_id, array $trail, int $gallery_id = 0 ): string {
		...
		ob_start();
		?>
		<nav class="mlgp-collection-breadcrumb" aria-label="<?php esc_attr_e( 'Navegacao da colecao', 'ml-gallery-pro' ); ?>">
	...
	private function sanitize_album_display_type( string $display_type ): string {
		...
		$allowed = [ 'grid', 'masonry', 'mosaic', 'tile', 'justified' ];
