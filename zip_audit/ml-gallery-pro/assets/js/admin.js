(function () {
	...
	function openAlbumPicker(type = "gallery") {
		state.albumPicker = {
			open: true,
	...
	function renderAlbumPickerItem(item, type) {
		...
		return `
			<div class="mlgp-linked-item mlgp-linked-item--picker">
				${cover}
	...
	function renderAlbumPickerModal() {
		if (!state.albumPicker || !state.albumPicker.open || !state.albumEditor.activeId) {
			return "";
		}

		const pickerType = state.albumPicker.type === "album" ? "album" : "gallery";
		const isGallery = pickerType === "gallery";
		const items = isGallery ? getAvailableAlbumGalleries() : getAvailableSubalbums();
		const title = isGallery ? "Adicionar galerias" : "Adicionar subalbuns";
		const description = isGallery
			? "Selecione galerias ja criadas para vincular ao album atual. A lista esta em ordem decrescente, das mais novas para as mais antigas."
			: "Selecione subalbuns validos para ampliar a navegacao sem criar ciclos. A lista esta em ordem decrescente, das mais novas para as mais antigas.";
