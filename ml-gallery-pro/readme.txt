...
* Impede cleanup_old_installations e filesystem writes durante frontend normal.
* Adiciona controle de versao instalada e lock curto de upgrade contra concorrencia.
* Sanitiza parametros publicos mlgp_album_view_* e mlgp_page_gallery_* com limite por request.
* Reduz carga causada por crawlers em paginas com multiplas galerias sem alterar a saida visual.

= 0.26.14 =
* Corrige a persistencia da capa escolhida ao salvar qualquer informacao ou configuracao da galeria.
* Corrige o atualizador GitHub herdado das versoes anteriores: elimina cache obsoleto, integra Verificar novamente e prioriza o asset versionado correto.
* Restaura compatibilidade real do updater com PHP 7.4 e preserva o estado de ativacao durante a atualizacao.
* Mantem slug, pasta, banco, frontend, editor, licenciamento e demais funcionalidades existentes.

= 0.26.12 =
* Corrige bug grave do lightbox que abria apenas o fundo escuro sem exibir a imagem.
* Isola a imagem do lightbox contra CSS global e mecanismos de lazy-load do tema.
* Adiciona carregamento visual, tratamento de erro e fallback automatico para a imagem grande quando o arquivo original falhar.

= 0.26.10 =
* Corrige pipeline de atualizacao automatica: o ZIP do plugin e agora anexado corretamente como asset da release no GitHub.
* Corrige after_install: renomeia a pasta extraida do source code zip do GitHub para o slug correto caso necessario.
* Esta versao e a primeira a ser entregue com o motor de atualizacao funcional de ponta a ponta.

= 0.26.9 =
* Adiciona GitHub Updater: o plugin agora aparece no painel WP Plugins > Atualizacoes quando ha nova versao disponivel no GitHub.
* Atualizacao automatica via botao nativo do WordPress sem necessidade de upload manual.
* Cache de verificacao: 6 horas quando ha versao disponivel, 15 minutos em caso de erro.
* Fallback de ZIP: usa asset da Release quando disponivel, ZIP automatico do GitHub como fallback.

= 0.26.8 =
* Corrige falha critica: lightbox agora possui navegacao completa (Anterior / Proximo).
* Adiciona suporte a teclado: seta esquerda = anterior, seta direita = proximo, Escape = fechar.
* Adiciona suporte a swipe/touch para navegar no mobile.
* Coleta automaticamente todos os itens da galeria ao abrir o lightbox (sem necessidade de fechar para ver a proxima imagem).
* Exibe contador "X / Y" no lightbox.
* Botoes Anterior/Anterior ocultados automaticamente quando ha apenas 1 item.
* Melhora visual do lightbox: botoes com SVG, stage centralizado, footer com caption e contador.
* Substitui document.body.style.overflow por classe CSS mlgp-lightbox-open para evitar conflitos.

= 0.26.7 =
* Corrige bloco Gutenberg para usar useBlockProps — o bloco agora e selecionavel, movivel, duplicavel e removivel como qualquer bloco nativo.
* Corrige bug de insercao duplicada: selecionar uma galeria/album atualiza o bloco atual em vez de inserir um novo.
* Muda BlockControls toolbar para exibir "Change Gallery" ou "Change Album" apenas apos selecao (nao mais Add Gallery/Add Album no toolbar vazio).
* Adiciona pointer-events:none no preview para que cliques cheguem ao wrapper do Gutenberg.
* Adiciona suporte a alinhamentos wide e full no bloco.
* Adiciona strings changeGalleryButton e changeAlbumButton no GalleryBlock.php.
* Atualiza block-editor.css com layout de preview-label, pointer-events fix e estilos do botao Change.

= 0.26.6 =
* Corrige categoria do bloco Gutenberg de "widgets" para "media" para aparecer na secao Media do inseridor.
* Expande keywords do bloco: gallery, album, photos, images, ml gallery pro.
* Preserva todo o comportamento de preview, insercao e sidebar existente.

= 0.26.5 =
* Adiciona botoes "Add Gallery" e "Add Album" no editor de blocos Gutenberg com modal de selecao direto no canvas.
* Adiciona botoes equivalentes na BlockControls toolbar do bloco para acesso rapido.
* Adiciona botoes secundarios no painel lateral (InspectorControls) para conveniencia.
* Localiza strings addGalleryButton, addAlbumButton, insertGalleryTitle, insertAlbumTitle e insertAction no GalleryBlock.php.
* Atualiza block-editor.css com estilos para os novos botoes e modal.

= 0.26.4 =
* Corrige docblocks orfaos no Ajax.php (deactivate_license, factory_reset, post() e regenerate_all_local_items).
* Corrige docblock errado em Plugin.php no metodo cron_fire_after_items_stored.
* Aplica sanitizacao de entrada (sanitize_text_field) nos campos de formulario admin-post da galeria e do album.
* Corrige logica do add_editor_buttons para nao passar pelo check de CPT quando a tela nao e editor de post.
* Sincroniza versao em todos os pontos exigidos pelo AGENT_RULES.md.
= 0.23.25 =
...
= 0.23.20 =
* restaura o botao Editar da lista de galerias para abrir o editor completo de gerenciamento
* ajusta o padrao de ordenacao para ID descendente quando nao houver preferencia salva
* remove o label visual do dropdown de ordenacao para manter a toolbar alinhada

= 0.23.19 =
* restaura o acesso direto a Licenca no menu e nas abas do painel sem alterar o motor de licenciamento
* normaliza status published como publish para leitura, gravacao e exibicao administrativa como Publicado
* melhora a listagem administrativa de galerias com toolbar alinhada, titulos menores e miniaturas quadradas

= 0.23.18 =
* corrige o texto do botao Voltar para a colecao no frontend sem reaproveitar rotulo antigo
* organiza o controle de ativar/desativar junto das opcoes do botao no painel
* preserva a heranca visual do tema quando as cores personalizadas estao vazias

= 0.23.17 =
* adiciona ordenacao visivel tambem na tela moderna de Gerenciar galerias
* remove chamada inexistente openGalleryCreator no fluxo Add Images
* completa o controle do botao Voltar para a colecao com borda e hover configuraveis sem herdar as cores do plugin

= 0.23.16 =
* adiciona controle administrativo do botao de navegacao do album no frontend
* permite ativar/desativar, alinhar, posicionar no topo/rodape/ambos e configurar cores sem sobrescrever o tema quando vazias
* preserva UTF-8/PT-BR e restaura a base administrativa da versao 0.23.14 antes de aplicar o controle do botao

= 0.23.15 =
* pacote descartado por regressao de encoding no admin
