...
if (root || config.isPostEditor) {
	...
	const state = {
		...
		albums: [],
		sorting: {
			galleries: (config.sorting && config.sorting.galleries) || "id_desc",
			albums: (config.sorting && config.sorting.albums) || "id_desc",
		},
		tags: {
			items: [],
			stats: {},
	...
	function normalizeGalleryStatus(status) {
	...
	function renderGalleryRow(item) {
		...
		return `
			<div class="mlgp-list__item mlgp-list__item--selectable mlgp-gallery-admin-row">
				...
				${cover}
				<div class="mlgp-gallery-admin-row__main">
					<h3>${escapeHtml(item.title)}</h3>
					<div class="mlgp-list__meta">
				...
				<div class="mlgp-actions mlgp-gallery-admin-row__actions">
					<a class="mlgp-button mlgp-button--ghost" href="${escapeHtml(managerUrl)}">Editar</a>
	...
	function renderGalleries() {
		...
		const filteredGalleries = (state.galleries || []).filter((item) => {
			...
			return haystack.includes(gallerySearchQuery);
		});
		...
		const visibleCount = filteredGalleries.length;
		const totalImages = state.galleries.reduce((sum, item) => sum + Number(item.item_count || 0), 0);
		...
		root.innerHTML = `
			<div class="mlgp-grid-two">
				...
				<section class="mlgp-panel">
					<div class="mlgp-panel-header-inline mlgp-gallery-list-header">
						<div>
					...
					<div class="mlgp-gallery-admin-toolbar">
						<div class="mlgp-gallery-admin-toolbar__counts">
							<span>${visibleCount} visiveis</span>
							<span>${state.galleries.length} galerias</span>
						...
						<div class="mlgp-gallery-admin-toolbar__tools">
							...
							<a class="mlgp-button mlgp-button--accent mlgp-gallery-admin-toolbar__create" href="${escapeHtml((config.pageUrls && config.pageUrls.addImages) || "#")}">Criar galeria com imagens</a>
						</div>
					</div>
					<div class="mlgp-actions mlgp-actions--bulk mlgp-gallery-bulk-actions">
					...
					<div class="mlgp-list">
						${filteredGalleries.length ? filteredGalleries.map(renderGalleryRow).join("") : '<div class="mlgp-empty">Nenhuma galeria encontrada.</div>'}
					</div>
				</section>
			</div>
		`;
	...
	function renderSettingsAlbumsTab(settings) {
		return renderSettingsSection(
			...
			`
				...
				<div class="mlgp-settings-callout">
					<strong>Botão de navegação do álbum</strong><br>
					Controle visual e posição do botão Voltar para a coleção no frontend do álbum.
				</div>
				<div class="mlgp-check">
					<input id="mlgp-album-nav-button-enabled" type="checkbox" name="album_nav_button_enabled" ${Number(settings.album_nav_button_enabled ?? 1) ? "checked" : ""}>
					<label for="mlgp-album-nav-button-enabled">Ativar botão Voltar para a coleção</label>
				</div>
				<div class="mlgp-settings-grid">
					<div class="mlgp-field">
						<label for="mlgp-album-nav-button-bg-picker">Cor de fundo do botão</label>
						<input id="mlgp-album-nav-button-bg-picker" type="color" data-mlgp-color-sync="album_nav_button_bg_color" value="${escapeHtml(settings.album_nav_button_bg_color || "#1f6f7a")}">
						<input id="mlgp-album-nav-button-bg-color" type="text" name="album_nav_button_bg_color" value="${escapeHtml(settings.album_nav_button_bg_color || "")}" placeholder="herdar do tema">
					</div>
					<div class="mlgp-field">
						<label for="mlgp-album-nav-button-text-picker">Cor do texto do botão</label>
						<input id="mlgp-album-nav-button-text-picker" type="color" data-mlgp-color-sync="album_nav_button_text_color" value="${escapeHtml(settings.album_nav_button_text_color || "#ffffff")}">
						<input id="mlgp-album-nav-button-text-color" type="text" name="album_nav_button_text_color" value="${escapeHtml(settings.album_nav_button_text_color || "")}" placeholder="herdar do tema">
					</div>
					<div class="mlgp-field">
						<label for="mlgp-album-nav-button-border-picker">Cor da borda do botão</label>
						<input id="mlgp-album-nav-button-border-picker" type="color" data-mlgp-color-sync="album_nav_button_border_color" value="${escapeHtml(settings.album_nav_button_border_color || "#1f6f7a")}">
						<input id="mlgp-album-nav-button-border-color" type="text" name="album_nav_button_border_color" value="${escapeHtml(settings.album_nav_button_border_color || "")}" placeholder="herdar do tema">
					</div>
					<div class="mlgp-field">
						<label for="mlgp-album-nav-button-hover-bg-picker">Cor de fundo no hover</label>
						<input id="mlgp-album-nav-button-hover-bg-picker" type="color" data-mlgp-color-sync="album_nav_button_hover_bg_color" value="${escapeHtml(settings.album_nav_button_hover_bg_color || "#1f6f7a")}">
						<input id="mlgp-album-nav-button-hover-bg-color" type="text" name="album_nav_button_hover_bg_color" value="${escapeHtml(settings.album_nav_button_hover_bg_color || "")}" placeholder="herdar do tema">
					</div>
					<div class="mlgp-field">
						<label for="mlgp-album-nav-button-hover-text-picker">Cor do texto no hover</label>
						<input id="mlgp-album-nav-button-hover-text-picker" type="color" data-mlgp-color-sync="album_nav_button_hover_text_color" value="${escapeHtml(settings.album_nav_button_hover_text_color || "#ffffff")}">
						<input id="mlgp-album-nav-button-hover-text-color" type="text" name="album_nav_button_hover_text_color" value="${escapeHtml(settings.album_nav_button_hover_text_color || "")}" placeholder="herdar do tema">
					</div>
					<div class="mlgp-field">
						<label for="mlgp-album-nav-button-align">Alinhamento do botão</label>
						<select id="mlgp-album-nav-button-align" name="album_nav_button_align">
							<option value="left" ${String(settings.album_nav_button_align || "left") === "left" ? "selected" : ""}>Esquerda</option>
							<option value="center" ${String(settings.album_nav_button_align || "left") === "center" ? "selected" : ""}>Centro</option>
							<option value="right" ${String(settings.album_nav_button_align || "left") === "right" ? "selected" : ""}>Direita</option>
						</select>
					</div>
					<div class="mlgp-field">
						<label for="mlgp-album-nav-button-position">Posição do botão</label>
						<select id="mlgp-album-nav-button-position" name="album_nav_button_position">
							<option value="top" ${String(settings.album_nav_button_position || "top") === "top" ? "selected" : ""}>Topo</option>
							<option value="bottom" ${String(settings.album_nav_button_position || "top") === "bottom" ? "selected" : ""}>Rodapé</option>
							<option value="both" ${String(settings.album_nav_button_position || "top") === "both" ? "selected" : ""}>Topo e rodapé</option>
						</select>
					</div>
				</div>
				<div class="mlgp-settings-grid">
	...
	function renderLicensePage() {
		root.innerHTML = `
			<section class="mlgp-panel mlgp-license-page">
				<div class="mlgp-panel-header-inline">
					<div>
						<h2>${escapeHtml(config.strings.licenseTitle || "Licenca / Serial")}</h2>
						<p class="mlgp-panel__intro">Acesso direto ao estado da licenca, validacao de serial e remocao local sem alterar o motor de licenciamento.</p>
					</div>
				</div>
				${renderLicensePanel(state.license)}
			</section>
	...
	function renderSettings() {
		...
		const tabRenderers = {
			...
			license: () => renderSettingsSection(
				config.strings.licenseTitle || "Licenca / Serial",
				"Gerencie o serial e o estado da licenca sem alterar o motor de validacao.",
				renderLicensePanel(state.license)
			),
	...
	async function loadAlbums(shouldRender = true) {
		const response = await request("mlgp_list_albums", { sort_mode: state.sorting.albums || "id_desc" });
		state.albums = response.items || [];
		state.sorting.albums = response.sort_mode || state.sorting.albums || "id_desc";
	...
	async function loadGalleries(shouldRender = true) {
		const response = await request("mlgp_list_galleries", { sort_mode: state.sorting.galleries || "id_desc" });
		state.galleries = response.items || [];
		state.sorting.galleries = response.sort_mode || state.sorting.galleries || "id_desc";
	...
	async function handleRootFieldChange(event) {
		...
		if (target.dataset.mlgpGallerySort) {
			state.sorting.galleries = target.value || "id_desc";
		...
		if (target.dataset.mlgpAlbumSort) {
			state.sorting.albums = target.value || "id_desc";
			clearAlbumSelection();
			await loadAlbums(false);
			renderAlbums();
			return;
		...
		if (target.dataset.mlgpColorSync) {
			const fieldName = target.dataset.mlgpColorSync;
			const linked = root.querySelector(`[name="${fieldName}"]`);
